---
author: 墨衡 (moheng)
created_time: 2026-05-31T10:32:00+08:00
type: experiment_plan_v2
...
| C5 | DB Schema变更 | ALTER TABLE + **DROP旧唯索引→创建含run_id的新唯索引** | 修复 | D2 | 5 min |
...
**改动点③** — `_build_null_record` 改为实例方法，使用 `self.source_version`（v2修复，原为`@staticmethod`硬编码`SOURCE_VERSION`）：
...
def _build_null_record(
    ...
    return {
        ...
        "num_stocks": n_stocks,
        "adjusted_ic": None,
        "source_version": self.source_version,  # ← v2修复：使用实例变量
        "created_at": datetime.now().strftime("%Y-%m-%dT%H:%M:%S+08:00"),
        "run_id": self.run_id,                  # ← 新增
    }
```

**改动点④** — 调用 `_build_null_record` 处（第503/521行）更新调用方式（实例方法调用无需额外参数）：

```python
# 原有调用（第503行）：
# null_record = self._build_null_record(date_std, factor_name, n_stocks)
# → 改为实例方法后调用方式不变（self.source_version和self.run_id在方法内部使用）
null_record = self._build_null_record(date_std, factor_name, n_stocks)
```
...
### 3.5 C4：Scheduler透传run_id + from_config修复
...
class ICBatchScheduler:
    def __init__(
        ...
        run_id: Optional[str] = None,           # ← 新增（v2修复D1）
    ):
        # ... 现有代码 ...
        self.run_id = run_id                     # ← 新增
```
...
**改动点③** — `from_config()` 工厂方法新增 `run_id` 参数（v2修复D1）：
...
def from_config(
    ...
    v2改动：新增run_id参数，透传到调度器和管线实例。
    """
    if db_config is None:
        db_config = {}

    db_path = db_config.get("db_path")
    max_connections = db_config.get("max_connections", 4)
    ...
    reg = FactorRegistry()
    engine = IC_Engine(db_manager=dm)
...
### 3.6 C5：DB Schema变更（v2修复D2）

#### 3.6.1 决策：唯一索引包含run_id（方案A）

**选择理由**：

选择 **方案A（唯一索引包含run_id）**，而非 玄知建议的方案B（date_range分段运行）。

| 对比维度 | 方案A：唯一索引含run_id | 方案B：date_range分段 |
|:---------|:----------------------|:---------------------|
| **对U2验证的意义** | ✅ 直接验证run_id作为行级标识的精确区分能力 | ❌ 规避了run_id在重复截面下的行为 |
| **代码改动量** | 2条SQL（DROP + CREATE） | 0条SQL改动（改流程） |
| **数据膨胀** | 每次重启新增一条相同截面记录（预期行为） | 各段互不重叠，无重复数据 |
| **与原始T+21故障的对齐度** | ✅ 高——原始故障中三次运行正是"覆盖相同截面集" | ❌ 低——分段运行不能复现覆盖写场景 |

**核心理由**：试验目标是**验证U2中run_id能否精确区分三次运行的写入量**，而三次运行（marine-s/mellow-o/norun）正是使用**相同的截面日期范围**（T0~T+21）。如果改用date_range分段，各段日期互不重叠，U2验证就失去了意义——原始故障场景本来就是"三个进程写了同一批截面"。方案A才是正确复现U2场景。
...
> SQLite中DROP INDEX后旧索引不存在，新索引 `(trade_date, factor_name, forward_window, source_version, run_id)` 使不同run_id的同一截面可共存，相同run_id的重复写入仍被INSERT OR IGNORE阻止。
...
### 3.7 C7：入口脚本改动（v2修复D1 + C1-2）

**新增脚本** `scripts/run_pipeline_experiment.py`：
...
parser.add_argument("--run-id", required=True, help="运行标识，如 marine-s / mellow-o / repro_p1")
parser.add_argument("--source-version", default="v1_repro")
parser.add_argument("--log-dir", default="pipeline_logs")
parser.add_argument("--min-memory", type=float, default=4.0,
                    help="启动时最低可用内存阈值（GB），默认4.0")
parser.add_argument("--resume-from", default=None,
                    help="恢复已中止的pipeline_run_id（checkpoint中的pipeline_id，"
                         "非run_id。通过查看checkpoint文件获取）")
parser.add_argument("--no-run", action="store_true",
                    help="跳过管线计算模式：仅写入run_id标记的汇总记录，"
                         "不执行IC计算。用于模拟--no-run场景下的写入")
parser.add_argument("--checkpoint-dir", default="schedules/checkpoints",
                    help="checkpoint目录，默认schedules/checkpoints")

args = parser.parse_args()

# ── run_id与pipeline_run_id的关系说明 ─────────────
# run_id（用户传入）→ DB写入标识，用于U2验证的GROUP BY
# pipeline_run_id（自动生成）→ checkpoint跟踪标识，用于resume
# 两者是独立的ID系统。--resume-from接受的是pipeline_run_id。
# 重启后新进程使用新run_id，通过--resume-from指向旧checkpoint实现恢复。
# ──────────────────────────────────────────────────────

def find_checkpoint_id(run_id: str, checkpoint_dir: str) -> Optional[str]:
    """通过run_id查找对应的checkpoint文件中的pipeline_id。
    约定：checkpoint文件名包含run_id的后缀。
    若找不到，用户需手动提供--resume-from。
    """
        ...
        if run_id in fname and fname.endswith(".json"):
            ...
            try:
                with open(fpath) as f:
                    ckpt = json.load(f)
                return ckpt.get("pipeline_id")
            except Exception:
                continue
    return None

# ── --no-run模式实现说明（v2修复C1-2） ─────────────
# 在--no-run模式下，管线不执行IC计算，仅模拟"第三阶段写入"：
# 1. 从已有数据库读取表a50_cross_ic_result中source_version='v1_repro'的截面日期范围
# 2. 对该范围内的每个截面，构造一条汇总标记记录（run_id=args.run_id, ic_value=NULL）
# 3. 写入后，后序查询GROUP BY run_id可统计到--no-run模式的写入量
#
# 此模式依赖已有数据的存在（需先完成正常重跑后执行），
# 不执行任何IC计算或因子计算。
# ──────────────────────────────────────────────────────

if args.no_run:
    print(f"[NO-RUN] Mode activated | run_id={args.run_id}")
    # 实现逻辑：从DB读取已存在的截面日期列表，
    # 对每个截面写入一条marker记录（每因子一条）
    # 详细实现见下方pseudo-code
    ...
...
dm = DatabaseManager(db_path="data/a50_ic.db")
reg = FactorRegistry()
engine = IC_Engine(db_manager=dm)
fr = ForwardReturns(db_manager=dm)
...
if args.resume_from:
    ...
    # 通过管线实例的run_batch_streaming的resume_from参数传入
    result_iter = pipeline.run_batch_streaming(
        args.start_date,
        args.end_date,
        args.step,
    ...
    scheduler.run_full_cross_sectional(
        args.start_date,
        args.end_date,
        args.step,
    )
...
| 1.3 | 手动模拟SIGKILL（用文件信号法，见§六）中断进程 | 进程终止 | 退出码137或进程消失 |
...
| P1.3 | SIGKILL后新run_id | 进程退出码为137（SIGKILL）或进程消失（通过wsl ps确认），无Python异常栈 | wsl ps + $? |
...
| **P1.9** | **null_record source_version验证**（v2新增） | `SELECT source_version, COUNT(*) FROM a50_cross_ic_result WHERE ic_value IS NULL AND source_version='v1_repro' GROUP BY source_version` → COUNT > 0 | SQL聚合确认v1_repro下存在null记录 |
| **P1.10** | **唯一索引不受run_id影响**（v2新增） | `SELECT trade_date, factor_name, forward_window, source_version, COUNT(DISTINCT run_id), COUNT(*) FROM a50_cross_ic_result WHERE source_version='v1_repro' GROUP BY trade_date, factor_name, forward_window, source_version HAVING COUNT(*) > 1` → 返回0行（相同run_id不重复，不同run_id共存） | SQL聚合确认相同五列中最多1行/同run_id |
...
- Phase 1 **全部10项PASS标准均通过**（含新增的P1.9和P1.10）
...
| 2.3 | 若被杀：查看checkpoint获取pipeline_id → `--run-id repro_p2 --resume-from <pipeline_id>` | 调度器检测到非零退出码后手动执行 |
...
| **P2.4** | **psutil监控记录（v2量化标准）** | **(a)** 日志中 `[MemGuard]` 标记出现次数 ≥ 总处理截面数 × 80%；**(b)** 日志可解析为CSV包含字段 [截面日期, phase, RSS_GB, Avail_GB] | 日志解析脚本统计 |
...
| 近5年 + 一次SIGKILL + resume | ~80 min | 被杀1次 + 重启耗时 |
| 近5年 + 两次SIGKILL | ~100 min | 最坏情况 |
| --no-run模式 | ~5 min | 跳过计算，仅写入 |

---

## 六、手动模拟SIGKILL方法（v2修复SIGKILL-1）

### 6.1 问题诊断

v1方案推荐 `taskkill /F /PID`，但该方案在WSL2中不可行——WSL2是轻量级虚拟机，从Windows侧看不到WSL2内部的Python进程。`Get-Process python` 不返回WSL2内部进程。

### 6.2 v2推荐方案：文件信号自杀（方案B）

**适用性**：WSL2和原生Linux均可靠，无需外部指令。

**实现**：在 `run_batch_streaming` 的每截面开头插入自杀检测逻辑：
...
```bash
# Step 1: 在另一个终端，创建信号文件触发自杀
touch schedules/checkpoints/KILL_NOW.signal

# Step 2: 观察管线日志，确认检测到信号后自杀
tail -f pipeline_logs/pipeline_*.log
# 应看到 [KillSim] KILL_NOW.signal detected ...
```
...
```bash
# 从WSL2 bash终端
ps aux | grep test_run1
kill -9 <PID>

# 或从Windows PowerShell一步到位
wsl ps aux | Select-String "test_run1"
wsl kill -9 <PID>
```

### 6.4 退出码验证（v2新增P1.3精确化）

| 场景 | 预期退出码 | 检查方式 |
|:-----|:----------|:---------|
| 文件信号自杀 | 137（128+9 SIGKILL） | `echo $?` |
| WSL内部kill -9 | 137 | `echo $?` |
| 正常完成 | 0 | `echo $?` |
...
## 九、--no-run模式实现说明（v2修复C1-2）

### 9.1 模式定位

`--no-run` 模式**不执行IC计算、不调用因子引擎**，仅用于模拟原始故障中的第三阶段（--no-run）写入场景。它负责在已有 `source_version='v1_repro'` 的数据库上，追加写入一批标记记录，使后序 `GROUP BY run_id` 可精确统计到该模式的写入量。

### 9.2 实现伪代码

```python
# 在入口脚本 run_pipeline_experiment.py 中
def run_no_run_mode(
    dm: DatabaseManager,
    run_id: str,
    source_version: str,
):
    """--no-run模式：跳过计算，仅写入run_id标记记录"""
    conn = dm.get()
    
    # 1. 获取已有v1_repro数据的截面日期+因子列表
    ...
    print(f"[NO-RUN] Wrote {len(existing)} markers for run_id={run_id}")
```

### 9.3 执行前提

- 必须先完成正常重跑（Phase 2.2/2.3），确保 `source_version='v1_repro'` 已有数据
- 应在正常重跑完成后再执行 `--no-run`
...
| R5 | WSL2环境差异导致复现假阴性 | 中 | 中——结论不可推广至原生Linux | 结论文档中明确标注环境差异（附录C已补充） |
...
| A7 | 方案是否考虑了WSL2 vs 原生Linux的环境差异（附录C v2补充） | 玄知（架构） |
...
-- A8: P1.9 null_record source_version一致性（v2新增）
...
| **SIGKILL行为（v2新增）** | WSL2中SIGKILL对Python进程子线程/子进程的终止时机 | ⏳ 待确认 | **影响U1结论可移植性**，结论文档需加注"WSL2环境观察" |
| **ulimit -v支持（v2新增）** | WSL2中 `ulimit -v` 是否有效 | ⏳ 待确认 | 若无效则OOM模拟方案C不可用 |
| **WSL2内存回收（v2新增）** | WSL2在释放后是否立即归还内存给Windows | ⏳ 待确认 | 影响 `Avail` 读数的有效性 |
| **跨文件系统IO（v2新增）** | DB/日志文件若是WSL2内部路径（`~/...`）vs Windows路径（`/mnt/c/`）| ⏳ 待确认 | 建议全部使用WSL2内部路径（如 `~/mozhi_platform/data/`）避免10x性能损失 |
| **psutil WSL2准确性（v2新增）** | `psutil.virtual_memory().available` 返回WSL2实例值还是宿主机值 | ⏳ 待确认 | **运行前先用 `python -c "import psutil; print(psutil.virtual_memory())"` 验证** |
...
| **SIGKILL-1** | 推荐 `taskkill /F /PID` 在WSL2中不可用 | 文件信号自杀法（方案B）为首选，WSL内部kill为备用 | §六 |
| **D1** | from_config未透传run_id；入口脚本无--resume-from；run_id与pipeline_run_id未对接 | from_config新增run_id参数；入口脚本新增--resume-from；补充ID系统说明 | C4、C7 |
| **C1-2** | --no-run模式无实现说明 | 新增§九完整实现描述 + 伪代码 | §九 |
| **C2-1** | 跳过截面时直接continue不yield，聚合器计数不准确 | 跳过时yield占位结果（含status=SKIPPED、memory信息、run_id） | C2、§3.3 |
...
| **D4** | 附录C遗漏5项环境差异 | 附录C补充SIGKILL行为、ulimit支持、内存回收、IO性能、psutil准确性 | 附录C |

---

*EOF — v2.0 | status=DRAFT | 等待墨萱+玄知再次评审*
